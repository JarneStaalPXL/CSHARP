...
namespace CS_Oefening_13
{
    ...
    public partial class Lid
    {
        ...
        private static WeekInschrijving[] weekOverzicht = WeekInschrijving.WeekOverzicht(9);
        ...
        public static List<Lid> LeesLeden(string bestand)
        {
            ...
            using (StreamReader sr = fi.OpenText())
            {
                while (!sr.EndOfStream)
                {
                    ...
                    sportTabel[lid.Sportcode].Aantal++;
                    sportTabel[lid.Sportcode].Totaalprijs += lid.TeBetalen;

                    // Update ons weekoverzicht na toevoeging van nieuw lid
                    int weekIndex = lid.Weeknr - 1;
                    weekOverzicht[weekIndex].Aantal++;
                    weekOverzicht[weekIndex].Bedrag += lid.TeBetalen;
                }
            }
            return leden;
        }
        ...
        public static string ToonWeekOverzicht()
        {
            ...
            {
                sb.AppendLine($"Week {i + 1} - {weekOverzicht[i].Aantal, 3} inschrijvingen: {weekOverzicht[i].Bedrag, 20:c}");
            }
        ...
        public static string ToonSporttakOverzicht()
        {
            ...
            foreach (var item in sportTabel)
            {
