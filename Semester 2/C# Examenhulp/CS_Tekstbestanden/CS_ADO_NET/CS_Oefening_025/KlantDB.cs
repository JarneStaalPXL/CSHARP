...
namespace CS_Oefening_025
{
    public static class KlantDB
    {
        ...
        public static void UpdateLaatsteKlant(Klant klant, string connectionString)
        {
            using (SqlConnection sqlcn = new SqlConnection(connectionString))
            {
                sqlcn.Open();
                string query = "select max(Klant_id) from Klant";
                SqlCommand cmd = new SqlCommand(query, sqlcn);
                klant.ID = (int)cmd.ExecuteScalar();
                ...
                updateCmd.Parameters.AddWithValue("@lastID", klant.ID);
                updateCmd.ExecuteNonQuery();
            }
        }

        public static void DeleteLaatsteKlant(string connectionString)
        {
            int lastID = 0;
            using (SqlConnection sqlcn = new SqlConnection(connectionString))
            {
                ...
                string deleteString = @"delete from Klant where Klant_id = @lastID";
                SqlCommand deleteCmd = new SqlCommand(deleteString, sqlcn);
                deleteCmd.Parameters.AddWithValue("@lastID", lastID);
